...
def _expand_path(path_str: str) -> str:
    """Convert shortcuts like 'desktop' to absolute paths."""
    shortcuts = {
    ...
    if path_lower in shortcuts:
        return str(shortcuts[path_lower])
    return path_str  # Return as-is if not a shortcut
...
TOOL_DECLARATIONS = [
...
},
    {
        ...
        "description": "Generates images from text prompts using Gemini image generation. Use when the user asks to create, draw, design, or generate an image.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "prompt": {"type": "STRING", "description": "Description of the image to generate"},
                "style": {"type": "STRING", "description": "Optional style like realistic, anime, cinematic, 3D"}
            },
            "required": ["prompt"]
        }
    },
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "open_app":
                ...
                # Expand path shortcuts before executing
                if "path" in args and args["path"]:
                    args["path"] = _expand_path(args["path"])
                if "destination" in args and args["destination"]:
                    args["destination"] = _expand_path(args["destination"])
                ...
                try:
                    img_client = genai.Client(api_key=_get_api_key())
                    response = await loop.run_in_executor(
                        None,
                        lambda: img_client.models.generate_content(
                            model="gemini-2.0-flash-preview-image-generation",
                            contents=f"{prompt}. Style: {style}"
                        )
                    )
